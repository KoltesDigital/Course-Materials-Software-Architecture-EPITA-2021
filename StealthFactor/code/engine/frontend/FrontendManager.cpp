...
namespace engine
{
	namespace frontend
	{
		...
		Manager::Manager(assets::Manager& assetManager, graphics::Manager& graphicsManager)
			: _assetProvider{ assetManager }
			, _context{ _assetProvider, graphicsManager }
		{
		}
		...
		void Manager::onMapLoaded(backend::Map& map)
		{
			...
			for (auto& character : map.characters)
			{
				...
				auto prefab{ _context.getAssetProvider().getAsset<Prefab>(prefabName) };
				if (!prefab)
				{
			...
			auto targetPrefab = _context.getAssetProvider().getAsset<Prefab>("prefabs/target.xml");
			...
			auto cameraPrefab = _context.getAssetProvider().getAsset<Prefab>("prefabs/camera.xml");
